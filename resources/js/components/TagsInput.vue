 ...
 * Midified Voerro Vue Tags Input v2 by Cristian Sorescu
 * 879091@swansea.ac.uk
 *
 * source: https://github.com/voerro/vue-tagsinput
 */
        ...
        <div :class="wrapperClass + ' tags-input'">
            <h5 class="mb-0">
              <span class="badge badge-primary"
                ...
                <span class="removeTag" @click.prevent="removeTag(index)">&times;</span>
              </span>
            </h5>
                ...
                if ((input.length && input.length >= this.typeaheadActivationThreshold) || this.typeaheadActivationThreshold == 0 || this.typeaheadAlwaysShow) {
                    ...
                    axios.get('/api/tags').then((response) => {
                        response.data.forEach((tag) => {
                            ...
                            const compareable = this.caseSensitiveTags ? newTag.value : newTag.value.toLowerCase();
                            if (compareable.search(searchQuery) > -1 && !this.tagSelected(newTag)) {
                                this.searchResults.push(newTag);
                            }
                        })
                    }).catch((error) => {
                        // something went wrong, show sample tags
                        for (let tag of this.existingTags) {
                            const compareable = this.caseSensitiveTags
                                ? tag.value
                                : tag.value.toLowerCase();
